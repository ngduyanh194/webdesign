...
function show_product(products) {
  ...
  for (var product of products) {
    product_list.innerHTML += `<div class="col-6 col-sm-4 col-lg-3 mt-3 p-3 product">
      ...
      <div class="text-end">
        <button onclick="add_to_cart(this.id)" id="${product.id}" class="btn btn-dark btn-sm">Add to Cart</button>
      </div>
    </div>
  `;
  }
...
function update_cart_quantity() {
  ...
  db.transaction(function (tx) {
    var 
    query = "SELECT SUM(quantity) AS total_quantity FROM cart WHERE account_id = ?";
...
function login_success() {
  ...
  var account_info = document.getElementById("account-info").innerHTML += `
  <button class="btn ms-3 disable btn-light ">Hello ${username}!</button>
    ...
    total += amount;
      cart_list.innerHTML += `
        <tr id="cart-list-item-${product.id}">
         ...
         <td>
            <button onclick="delete_cart_item(this.id)" id="${product.id}" class="btn btn-danger btn-sm center">Delete</button>
         </td>
        </tr>
      `;
  }
